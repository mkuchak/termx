...
## 16. Mosh comes up on-device — and the diagnostic Copy/Share works when it doesn't
- Click-path: on a server with mosh enabled and the UDP port range reachable, FORCE-STOP the app first (real cold start), reopen, connect. Watch the transport subtitle.
- Expected (v1.7.7+): the session connects via **mosh** — no "via SSH — mosh unavailable: …" subtitle, no fallback snackbar. The on-device proof of the static-binary fix: if you DO get a fallback and tap the subtitle, the `mosh diagnostics` dialog must show `exit=0` (clean) — never the old `exit=139` (SIGSEGV) that the bundled Termux binary produced before it ever reached `main()`.
- Reconnect (v1.7.8+): once mosh is up, **Disconnect → Reconnect 3–5 times in a row** (prefer a `herdr`/tmux-backed session so the remote multiplexer persists across reconnects — that's the case that used to break). EVERY reconnect must render the **live** screen IMMEDIATELY — no freeze, no garbled or stale screen, and no stuck "mosh: Nothing received from server on UDP port NNNNN" overlay. Type after each reconnect and confirm the screen tracks your input.
- Mouse / scroll (v1.7.9+): in a **mosh** session running a multiplexer that uses mouse mode (`herdr`/tmux/zellij), **drag a finger to scroll** the active pane and **tap a different pane to switch to it**. Scrolling must scroll the pane (or its copy-mode) — it must NOT recall the agent's previous prompts (that's the old scroll→arrow-key fallback), and a tap must move focus between panes. Re-run the identical check over SSH; both transports must behave the same. Before v1.7.9, mosh dropped into mouse-off: scroll fired Up/Down arrows (history recall) and taps did nothing, while SSH worked.
- Would have caught: the v1.7.8 mosh keyframe-drop bug — `MoshRemoteTerminalSession` dropped mosh's first frame (the full-screen keyframe) because the emulator wasn't mounted yet, and mosh's delta stream can't recover a lost base. Only the FIRST connect rendered (a cold 80×24→real-size resize accidentally forced a full repaint); every RECONNECT attached to the already-sized herdr session, the resize was a no-op, no repaint happened, and the screen froze. Fixed by buffering pre-emulator bytes and replaying them on `initializeEmulator` (gotcha #33).
- If it falls back with the static binary: the subtitle reads "via SSH — mosh unavailable: mosh-client produced no output · tap for details". **Tap it** → the `mosh diagnostics` dialog (device/OS/ABI/**page-size** + exit code + captured stdout/linker log); **Copy** must reach the clipboard (paste-verify into a notes app), **Share** must open the share sheet. With the static binary a non-zero exit or a real linker line here is a NEW regression — investigate (cross-check `adb logcat | grep -iE 'mosh|MoshSession|linker'` if you have a cable).
- Would have caught: the v1.7.4–v1.7.6 saga — timeout/bind/message tweaks could never fix a binary that **SIGSEGV'd before `main()`** (the 53-`.so` Termux bundle died in a static-init/ODR crash in the dynamic linker, logged to logcat not the pty). v1.7.7 rebuilt mosh-client as a single static PIE (bionic-only deps), eliminating the crash class; the v1.7.6 dialog is what proved it (`exit=139`@19 ms → must now be `exit=0`). §5.5, §5.8, gotcha #32.
