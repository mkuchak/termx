...
- Expected: the session connects via **mosh** (no "via SSH — mosh unavailable: …" subtitle, no fallback snackbar).
- If it DOES fall back: the subtitle must read "via SSH — mosh unavailable: mosh-client produced no output · tap for details" (never the old "check firewall" or "slow start / blocked UDP" text). **Tap it** → a `mosh diagnostics` dialog opens with device/OS/ABI/**page-size** + the mosh-client exit + its captured stdout/linker log; **Copy** must put that on the clipboard (paste-verify into a notes app) and **Share** must open the share sheet. This is the no-adb path to the real failure — a `library "…_mosh.so" not found` / `cannot locate symbol` / `p_align` line in the blob names the root cause directly. (Cross-check vs `adb logcat | grep -iE 'mosh|MoshSession|linker'` if you have a cable.)
- Would have caught: the v1.7.5 mis-diagnosis — the timeout bump (3 s → 15 s) did NOT fix mosh because the client dies *pre-`main()` in the dynamic linker* (logged to logcat, not the pty), not from slowness. A running mosh-client paints within ms, so zero output ⇒ a load failure. v1.7.6 surfaces that captured linker log via the Copy/Share dialog instead of the misleading "blocked UDP" message (§5.8, gotcha #32).
