...
## 16. Mosh actually comes up on-device (cold-start, not a silent SSH fallback)
- Click-path: on a server with mosh enabled and the UDP port range reachable, FORCE-STOP the app first (so the next launch is a real cold start — this is the condition that broke), reopen, and connect. Watch the transport indicator/subtitle.
- Expected: the session connects via **mosh** (no "via SSH — mosh unavailable: …" subtitle, no fallback snackbar). It may take a few seconds on the first cold connect while the native mosh-client link-loads — that is normal and must NOT trip the fallback. Reconnecting a second time (warm) should be fast.
- If it DOES fall back: the message must read "no response in time — slow start or blocked UDP" — never the old "check firewall" text. Then diagnose properly: `tcpdump` the SERVER (`sudo tcpdump -n -i any 'udp and portrange 60000-60010'`) while tapping connect. ZERO packets arriving = the phone never transmitted (client cold-start/timeout, NOT the firewall); packets arriving but no reply = genuinely a server-side UDP block. Grab `adb logcat | grep -iE 'mosh|MoshSession'` for the client side.
- Would have caught: the v1.7.5 mosh cold-start bug — the 3 s first-output gate killed the native mosh-client mid-startup (~40 `lib*_mosh.so` deps) before it sent a single UDP datagram, so mosh fell back to SSH every time on a cold process while working instantly from a desktop client. Fixed by the 15 s gate; the bind also dropped IPv4-only `-i 0.0.0.0` so `-s` follows the SSH address family (§5.5, §5.8, gotcha #32).
